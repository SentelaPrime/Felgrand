...
function s.equipop(c,e,tp,tc)
	...
	--atk/def
    local lvrk=math.max(tc:GetOriginalLevel())
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_DECK|LOCATION_HAND|LOCATION_GRAVE,0,1,1,nil,c)
	local tc=g:GetFirst()
	if tc then
		s.equipop(c,e,tp,tc)
	end
